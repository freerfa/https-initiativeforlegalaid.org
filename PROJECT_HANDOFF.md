...
> Last updated: 2026-09-24. Status: **LIVE at https://initiative4legalaid.org/ (cutover COMPLETE — verified 2026-09-23/24: apex A → 216.24.57.1, www CNAME → Render, HTTP/2 200, TLS issued)** + Render service https://https-initiativeforlegalaid-org-p51u.onrender.com (+ local dev http://127.0.0.1:5001). Production domain decided 2026-09-23: **`initiative4legalaid.org` (with "4"), registered at Hostinger** — final zone + what-actually-happened recorded at the top of `deploy/DNS_CUTOVER.md`. Old `initiativeforlegalaid.org` (OraWebHost/Truehost) is no longer the target; its mail (MX/SPF) stays untouched and unaffected.
...
12. 2026-09-15 Domain DNS typo risk. Symptom: user added `initiative4legalaid.org` (with "4") as custom domain in Render — domain did not exist then. ORIGINAL fix was to remove the entry and use `initiativeforlegalaid.org` (f-o-r). **UPDATE 2026-09-23: the user has since REGISTERED `initiative4legalaid.org` at Hostinger, and it is now the decided production domain — keep/verify that Render custom-domain entry instead of deleting it. See the CURRENT PLAN section at the top of deploy/DNS_CUTOVER.md. UPDATE 2026-09-24: CUTOVER DONE — https://initiative4legalaid.org/ is live (HTTP/2 200, TLS issued). The first attempt appeared to fail because the apex sat on `ALIAS @ → initiative4legalaid.org.cdn.hstgr.net` (Hostinger CDN), not on parking A records, so the placeholder kept serving; the Render IP had also been entered on an `A ftp` record by mistake. Resolved by deleting the ALIAS, adding `A @ → 216.24.57.1`, and removing the stray ftp record.**
13. 2026-09-15 Mail-breakage risk during DNS cutover. Symptom: none yet (prevented). Cause: MX + mail A + SPF records for initiativeforlegalaid.org live on the OLD OraWebHost server; a naive "change all A records" cutover would kill @initiativeforlegalaid.org email. Fix: DNS_CUTOVER.md now warns: change ONLY `A @` → 216.24.57.1 and `CNAME www` → https-initiativeforlegalaid-org-p51u.onrender.com; never touch mail/MX/SPF. Also: mailboxes (info@, emmanuel@, nicodemus@, modi@) must be created in OraWebHost cPanel. Verify: `dig MX initiativeforlegalaid.org` still returns mail host after cutover; test send/receive. NOTE 2026-09-24: the cutover was executed on the DIFFERENT zone `initiative4legalaid.org` (Hostinger), so this risk did not materialise — `initiativeforlegalaid.org` mail records were never touched. Lesson for future moves: on the cutover domain, delete/replace ONLY the apex web record; leave `MX`, `TXT` (SPF/DKIM), `autodiscover`, `autoconfig` rows alone.
14. 2026-09-24 Cutover "looked done" but the placeholder kept serving. Symptom: user reported the apex A record changed, yet `dig` from Google/Cloudflare still returned rotating Hostinger IPs (88.222.x / 84.32.x / 2.57.91.x) and the site title stayed "Default page". Cause: the apex was an `ALIAS @ → initiative4legalaid.org.cdn.hstgr.net` (Hostinger CDN) — editing/adding an A record alongside it does not take effect, and the Render IP had actually been saved on a different record name (`A ftp`). Diagnosing by IP alone was misleading because Hostinger CDN returns a pool of addresses. Fix: in Hostinger hPanel delete the `ALIAS @` row, add `A @ → 216.24.57.1`, and delete the stray `A ftp` row. Verify: `dig @8.8.8.8 +short A initiative4legalaid.org` → 216.24.57.1 and `curl -s https://initiative4legalaid.org/ | grep -o '<title>[^<]*</title>'` → "Initiative for Legal Aid South Sudan" (no longer "Default page"). LESSON: check the RECORD TYPE (`ALIAS`/`ANAME` vs `A`) and the RECORD NAME (`@` vs `ftp`) in the provider's DNS table, not just the IP value.
...
## 10. Production checklist (as of 2026-09-24)
...
- [x] **DNS cutover for `initiative4legalaid.org` at HOSTINGER hPanel — DONE + verified 2026-09-23/24.** Actual change differed from the plan: the apex was on `ALIAS @ → initiative4legalaid.org.cdn.hstgr.net` (Hostinger CDN), NOT on the parking A records — deleting that ALIAS and adding `A @ → 216.24.57.1` fixed it (a first attempt put the Render IP on an `A ftp` record by mistake; cleaned up). `CNAME www → https-initiativeforlegalaid-org-p51u.onrender.com` was already correct. Final zone + commands in deploy/DNS_CUTOVER.md CURRENT PLAN section.
- [x] **Render Custom Domains — DONE:** both `initiative4legalaid.org` + `www.initiative4legalaid.org` verified on service srv-dakep4bm8hqs73ebt5t0; TLS auto-issued (CN=initiative4legalaid.org, Google Trust Services WE1, valid to 2026-12-22). (The 2026-09-15 "typo" concern is resolved — the "4" domain now exists and is the production target, see §8 #12 update.)
...
- [x] Post-cutover verification — DONE 2026-09-24: `dig @8.8.8.8 A initiative4legalaid.org` → 216.24.57.1; `dig @8.8.8.8 CNAME www.initiative4legalaid.org` → ...onrender.com; `curl -L https://initiative4legalaid.org/` → 200 on HTTP/2, final URL the apex; `<title>` = "Initiative for Legal Aid South Sudan"; TLS valid. `dig MX initiativeforlegalaid.org` untouched (its mail is separate, on OraWebHost).